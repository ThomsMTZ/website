...
const ContactForm = () => {
  ...
  const [mountedAt, setMountedAt] = useState(0); // anti-bot timestamp
  ...
  const handleSendMail = async e => {
    ...
    try {
      ...
      const msg =
        err?.response?.data?.message || err?.response?.data?.error || t.contact.form.error;
      toast.error(msg);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div>
      <p className="font-medium mb-5 text-[#16f2b3] text-xl uppercase">{t.contact.heading}</p>
      <div className="max-w-3xl text-white rounded-lg border border-[#464c6a] p-3 lg:p-5">
        ...
        <form className="mt-6 flex flex-col gap-4" onSubmit={handleSendMail}>
          ...
          <div className="flex flex-col items-center gap-3">
            {error.required && <p className="text-sm text-red-400">{t.contact.form.required}</p>}
            <button
              type="submit"
